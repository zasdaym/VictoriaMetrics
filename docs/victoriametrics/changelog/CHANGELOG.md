...
* FEATURE: `vmstorage` and `vmselect`: expose the `vm_app_started_after_unclean_shutdown` gauge. It is set to `1` when the previous process run didn't shut down cleanly. `vmselect` reports it when `-cacheDataPath` is set. Added the `UncleanShutdown` [alerting rule](https://github.com/VictoriaMetrics/VictoriaMetrics/blob/master/deployment/docker/rules/alerts-cluster.yml), which fires for 10 minutes after a detected unclean shutdown. See [#8443](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/8443).
